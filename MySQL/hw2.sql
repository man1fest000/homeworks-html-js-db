...
from client
         join department d on d.idDepartment = client.Department_idDepartment
where DepartmentCity is not null
union
select count(*)
from client
...
select idClient, Client_idClient, FirstName, LastName, Sum
from client
         join application a on client.idClient = a.Client_idClient
where Sum = (select max(Sum) from application where Client_idClient = client.idClient);
